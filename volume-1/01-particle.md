...
물리학에서 “입자”라는 말은 일상어의 작은 알갱이와 완전히 같지 않다. 전자는 검출기에서 한 번의 상호작용으로 에너지와 전하를 전달하고, 표준모형에서는 기본적인 물질입자(lepton)[^v1c01-lepton]로 분류된다. 현대 입자물리의 표준 기술은 전자를 내부 부품을 가진 작은 고전적 공으로 다루지 않는다.

하지만 여기서 조심해야 한다. “현재 실험과 이론이 전자를 점입자처럼 다룬다”는 문장과 “전자에는 어떤 더 깊은 내부 자유도도 절대로 있을 수 없다”는 문장은 논리적으로 다르다. 실제로 입자물리학은 더 작은 substructure[^v1c01-substructure]가 있는지를 계속 시험하며, 발견되지 않았다는 사실과 존재 불가능의 증명은 구분한다.
...
| **1. 관측 언어**                                                                   |
|------------------------------------------------------------------------------------|
| 현재 실험에서 내부 크기나 form factor[^v1c01-form-factor]를 분해해 보지 못해 “점처럼 보인다”고 말한다. |
...
v0.1은 가능한 내부 자유도를 넓게 열어 두었다. 스칼라장, 스피너장, 게이지장, 위상적 양자수 같은 후보를 한꺼번에 검토하면서, 가장 먼저 해결해야 할 문제로 “왜 전자가 실험에서 점입자처럼 보이는가?”, “로런츠 대칭[^v1c01-lorentz-symmetry]과 충돌하지 않는가?”, “전하·질량·스핀을 실제로 유도할 수 있는가?”를 등록했다.
...
[^v1c01-lepton]: 전자·뮤온·타우와 중성미자들을 묶는 표준모형의 기본입자 계열. 이 장에서는 전자가 이 계열에 속한다는 정도만 기억하면 된다.

[^v1c01-substructure]: 겉으로 하나로 보이는 대상 안에 더 작은 구성요소나 내부 자유도가 있는 구조를 뜻한다. “아직 발견되지 않음”과 “존재할 수 없음”은 서로 다른 말이다.

[^v1c01-form-factor]: 산란 실험에서 대상의 전하나 내부 분포·크기 같은 구조가 드러나는 방식을 나타내는 양. 점처럼 보이는지 시험할 때 쓰이는 대표적인 도구 중 하나다.

[^v1c01-lorentz-symmetry]: 서로 일정한 속도로 움직이는 관측자들이 물리 법칙을 같은 형태로 기술해야 한다는 특수상대론의 핵심 대칭. 여기서는 “후보 내부구조가 상대론과 모순되지 않는가?”라는 연구 질문을 가리킨다.
